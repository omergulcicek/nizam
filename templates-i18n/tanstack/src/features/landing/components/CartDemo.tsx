    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-4 rounded-xl border border-border/50 bg-background/50 p-3">
        ...
        <div className="flex flex-1 flex-col">
          <h5 className="font-medium text-foreground">{m.cart_item_name()}</h5>
          <p className="text-sm text-muted-foreground">{m.cart_item_variant()}</p>
        </div>
        <div className="flex flex-col items-end gap-1">
          <span className="font-semibold text-foreground">{count * 350}</span>
          <span className="text-xs text-muted-foreground">{m.cart_item_total()}</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <span className="text-xs">{m.cart_clear()}</span>
        </Button>
      </div>
    </div>
  );
